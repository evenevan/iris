...
export function detailMessage(apiData, userOptions) {
  ...
  if (apiData?.isOnline === true) {
    ...
    } else if (apiData?.offline?.lastGame) playerDataString += `<br><strong>Last Game Type:</strong> ${apiData?.offline?.lastGame}`;
  }

  //eslint-disable-next-line default-case, no-extra-parens
  ...
  function cleanTimeStamp(time, date, ms) {
    ...
    return `${time}, ${date}${ms ? `<br>&nbsp;&#8627; ${cleanTime(timeAgo(ms))} ago` : ""}`;
  }
