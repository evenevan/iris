...
const testAPIKeyResultSpan = document.getElementById('testAPIKeyResult') as HTMLSpanElement;

const testAPIKeyLoading = document.getElementById('testAPIKeyLoading') as HTMLDivElement;
...
testAPIKeyButton.addEventListener('click', async () => {
    ...
    switch (request?.status) {
        case 200:
            {
                const uses = (await Request.tryParse<{
                    record: {
                        totalQueries: number;
                    };
                }>(request))!.record.totalQueries;

                testAPIKeyResultSpan.textContent = runtime.i18n.getMessage(
                    'settingsHypixelAPI200',
                    String(uses),
                );
            }
            break;
